...
export const describeWarning = (warning: Warning): string => {
  ...
  switch (warning.code) {
    ...
    case "wash_sale_window_prior_buy":
      return `Venta con pérdida de ${text(d.asset_id)} del ${text(d.sale_date)} (${text(d.loss_eur)} EUR) con una compra del ${text(d.buy_date)} de ${text(d.buy_quantity)} títulos que sigue en cartera, dentro de la ventana abierta el ${text(d.window_start)} (${windowText(d.window)}): la pérdida puede no ser computable en ${text(d.tax_year)}. Cuánto, lo dice \`atlas tax ${text(d.tax_year)}\` (business-rules.md §5.4).`;
