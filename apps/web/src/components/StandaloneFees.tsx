...
// leaving the account all the same, and until feature 007 no screen of either
// interface showed them at all (Q9).
//
// One component, used by both screens, because each book shows **its own**
// charges and its own total: the core's on Núcleo, the bucket's on Cubo
// (constitution III).
...
export interface StandaloneFeesProps {
  ...
  /** How the total names its book: "Total del núcleo", "Total del cubo". */
  totalLabel: string;
}
...
export const StandaloneFees = (props: StandaloneFeesProps): JSX.Element => (
  <Show when={props.view.rows.length > 0}>
    ...
    <div class="spread total-line">
      <span class="subject">{props.totalLabel}</span>
