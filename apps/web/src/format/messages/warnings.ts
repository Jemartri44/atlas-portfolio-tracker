...
export const WARNING_MESSAGES: Record<string, (d: Details, n: Naming, f: Figures) => string> = {
  ...
  wash_sale_window_prior_buy: (d, n, f) =>
    `Venta con pérdida de ${n.one(d.asset_id)} del ${day(d.sale_date)} (${f.money(d.loss_eur)}) con una compra del ${day(d.buy_date)} de ${f.quantity(d.buy_quantity)} títulos que sigue en cartera, dentro de la ventana abierta el ${day(d.window_start)} (${windowText(d.window)}): la pérdida puede no ser computable en ${num(d.tax_year)}.`,
