...
**Consecuencias.** Los libros dorados no se movieron (la única corrección de `synthetic-v1.jsonl` es un dividendo, que no abre lote y está solo en su fecha). Los lotes llevan la posición de su raíz (`FiscalLot.position`). Un bucle de correcciones (`A→B, B→A`) solo puede venir de un fichero editado a mano, y no tiene primer evento: **cada miembro del bucle es su propia raíz**. Lo que la proyección hace con él depende de qué anulaciones haya en el fichero: sin anulaciones, cada miembro es una corrección colgante (`dangling_correction`); con las dos, no se rechaza nada y los dos quedan inactivos, porque están anulados. Y la raíz de una corrección que apunta a un miembro del bucle **depende del orden del fichero**: si el miembro aparece antes, hereda su raíz; si aparece después, es su propia raíz.
