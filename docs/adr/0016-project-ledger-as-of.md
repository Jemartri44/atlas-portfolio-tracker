...
- *Precisión añadida el 2026-09-18, tras la revisión de la feature 005:* lo que se proyecta en la **primera** pasada y no tiene fecha de negocio —las tesis del cubo— **no queda cortado por `asOf`** y hay que filtrarlo aparte, por su fecha administrativa (`docs/data-schema.md` §7.1). Se descubrió porque una vista a una fecha pasada listaba tesis que aún no existían, con días abiertos negativos, y promediaba estadísticas sobre tesis que no habían operado. La regla general queda así: **`asOf` corta hechos de negocio; los documentos administrativos se filtran por su propia fecha**.
