...
| 5 | **Retención en venta forzosa** | `withholding?` **dentro de `per_account[]`**, no uno por evento | Liquidación de fondo, venta de derechos, ETC en bróker español |
...
| 9 | **`fx_rate_date` pasa a obligatorio** en `cash_deposit`, `cash_withdrawal`, `standalone_fee` **y `valuation`** | Endurecimiento, permitido por ADR-0018 **solo mientras el libro real esté vacío** | Lotes de divisa reproducibles (criterio 4) y, en `valuation`, el tipo con el que el Modelo 720 convierte la cotización de 31/12 — el dato que ADR-0013 dice que no se puede perder |

> **Enmienda del 2026-09-18 (misma fecha), previsión 5.** El texto original decía «misma forma que `sell.withholding`», es decir una retención por evento. **Es incorrecto**: un `forced_sale` liquida **cuenta a cuenta** y su `fee` ya vive en `per_account[]` desde el hallazgo 8 del *challenge* 2, así que una retención única no podría repartirse entre las cuentas que venden. Lo detectó el implementador de la 008 al planificar. Se corrige el mismo día en que se detecta, que es la regla que dejó ADR-0013 tras pasar tres semanas contradiciendo al código.

> **Nota del 2026-09-19, previsión 1.** El valor por defecto de `income_category` deja de ser `capital_gain` para **`etc` y `etp`**, que pasan a **`movable_capital`**: el criterio **#24** de `docs/fiscal-questions.md`, fundado en la consulta vinculante de la DGT V0267-25 (13/03/2025), califica el rendimiento de un ETC como rendimiento del capital mobiliario «en todo caso» (certeza alta), y el de un ETP igual por defecto (certeza media: depende de la estructura de cada producto). El cambio se hace en el bloque 0 de la feature 010. Es exactamente para lo que existe esta previsión: la duda se resuelve cambiando un valor, no el esquema. La frase «no cambia ninguna cifra que el sistema calcule hoy» de abajo describe el momento de esta decisión; desde la 010, los ETC y ETP compensan como rendimientos. Y por ADR-0022, un libro cuyo `settings_changed` ya fija `capital_gain` para esos tipos lo conserva hasta que un cambio de configuración diga otra cosa.
