...
**Lo que este documento no garantiza, dicho para no afirmar lo que el código no cumple** (corregido el 2026-09-23 tras la revisión): que una escritura **concurrente** entre la comprobación del etag y la escritura se detecte depende del **adaptador**. **Los dos almacenes que se usan de verdad tienen esa ventana**, no solo uno. El almacén `blob` —el de la web, sobre el navegador o sobre una carpeta— compara el etag al leer los bytes actuales y después escribe **sin condición**. El almacén de **fichero** —el de la consola, que es el que el usuario usa de verdad— hace lo mismo: `currentBytes` compara el etag y, después, `replace` y `append` escriben un temporal y lo renombran sobre el libro, sin ningún cerrojo entre la comprobación y el renombrado. En los dos, otra escritura en ese intervalo (otra pestaña, otra consola, la web sobre la misma carpeta) se pisaría. El renombrado hace atómica la **sustitución** del fichero, no la **comparación más la sustitución**. Es anterior a esta decisión, afecta a toda escritura y no solo a la renuncia, y queda anotado para una ronda siguiente, **para los dos adaptadores**; esta ronda no los toca. (Ampliado el mismo día tras la segunda revisión: la primera corrección solo nombraba el `blob`.)
