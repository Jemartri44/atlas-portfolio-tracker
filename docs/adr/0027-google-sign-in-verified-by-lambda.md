...
**Y una restricción propia:** la constitución prohíbe la analítica y las CDN de terceros y exige una CSP restrictiva (Restricciones técnicas, Seguridad); la especificación pide servir **todo** desde el propio origen y lo razona en §10: un *script* de terceros en una aplicación financiera es una vía de exfiltración. La CSP de producción es `script-src 'self'` y `connect-src 'self'` (`apps/web/index.html`), y `npm run build` falla si el resultado contiene una URL a un origen ajeno (README, «CSP: desarrollo y producción»).
...
- Documentos que hay que actualizar: la constitución (Restricciones técnicas: «Cognito con MFA» → acceso con Google verificado en la Lambda), §9.2, §9.3, **§9.6** (nombra Cognito y el modo privacidad «activado por defecto al iniciar sesión») y §10 de la especificación, la tabla de *stack* y la de seguridad de `CLAUDE.md`, una nota fechada en ADR-0019, y el traspaso de dirección, con el requisito operativo de la verificación en dos pasos.
