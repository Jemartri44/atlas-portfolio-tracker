...
**Estado:** Propuesta (2026-09-24). Ronda 8. La decisión de fondo es de la dirección; este documento la registra con su motivo y propone **cómo** obtener el token sin romper la CSP, que es lo que la dirección no fijó. **Sustituye** a «Cognito con MFA» de la constitución (Restricciones técnicas), de `docs/specification.md` §9.2, §9.3 y §10, de la tabla de *stack* de `CLAUDE.md` y de ADR-0019 («Cognito protegerá la API»), cuando se acepte.
...
- **c. Redirección con código de autorización y PKCE, con la Lambda como cliente OAuth** (recomendada). La SPA navega a `/api/auth/login`; la Lambda redirige a Google; Google vuelve a `/api/auth/callback` con un código; la Lambda lo canjea por el ID token directamente con Google y lo verifica. *Ventajas:* el token **nunca toca la SPA** ni la URL; el retorno es un `GET` sin cuerpo, así que el requisito del hash no aplica; la SPA no necesita conocer el identificador del cliente, de modo que cambiarlo (ver riesgos) es cambiar un parámetro de SSM, no reconstruir la web. *Inconvenientes:* un secreto más (el secreto del cliente OAuth) en SSM, y una llamada saliente de la Lambda a Google en cada inicio de sesión.

## Decisión

Se adopta el acceso **solo con Google, verificado en la Lambda**, y se propone obtener el token por la **vía c**; la **b** queda como alternativa para la dirección. La **a** no.
...
**Por dónde viaja cada cosa, por el hecho 1 de arriba:** la sesión, en la cookie; si se elige la vía b, el token de Google, en una cabecera propia. **Nunca en `Authorization`**, que CloudFront sobrescribe. Todo `POST` y `PUT` de la SPA lleva `x-amz-content-sha256`, calculado con Web Crypto.
...
- **Lo que se pierde frente a Cognito, dicho claro:** la aplicación **no puede comprobar** que la cuenta de Google tenga activada la verificación en dos pasos, ni impedir que Google recuerde el navegador. «MFA obligatorio» pasa a depender de cómo mantenga el usuario su cuenta de Google. Si Google permite forzar la reautenticación en cada acceso con los parámetros de OpenID Connect: **SIN VERIFICAR**.
...
- **Secretos nuevos en SSM:** el secreto del cliente (vía c) y la clave de sesión. La frase «el único secreto es el token Flex de IBKR» de `CLAUDE.md` y de §10 de la especificación deja de ser cierta, y hay que corregirla al aceptar (junto con las claves de las fuentes de precios de ADR-0031).
- Al aceptar: enmienda de la constitución (Restricciones técnicas: «Cognito con MFA» → acceso con Google verificado en la Lambda), §9.2, §9.3 y §10 de la especificación, la tabla de *stack* y la de seguridad de `CLAUDE.md`, y una nota fechada en ADR-0019.
