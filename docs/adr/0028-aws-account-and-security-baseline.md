...
- **Fila 9** (Budgets): se mantiene a 1 $ y antes de créditos, **filtrada por la etiqueta de asignación de costes `project=atlas`**, que se activa a mano en Billing. Budgets sin acciones es gratuito sea cual sea el número de presupuestos (ADR-0034, F1).
...
- **Fila 17** (SES en *sandbox*): el *sandbox* es por cuenta y región, y otro proyecto puede haber sacado ya la cuenta de él. **Su salvaguarda la sustituye una condición de IAM** sobre el remitente y el destinatario (ADR-0034, fila 12).
- **Bloque «Terraform»**: el *bootstrap* se aplica **una vez para la cuenta**, no una por cuenta miembro, y el estado vive en **un bucket propio de Atlas** con una clave por entorno (ADR-0034, filas 15 y 20).
- **Excepciones escritas**: desaparecen «la organización y las dos cuentas miembro»; «el *bootstrap* de Terraform de cada cuenta» pasa a ser uno; entra la **activación de la etiqueta de asignación de costes** (ADR-0034, fila 20).
- **Consecuencias**: la afirmación sobre las SCP y la cuenta de gestión y la advertencia sobre la SCP de regiones **quedan sin objeto**. Y **queda contestado un SIN VERIFICAR**: al superar lo incluido en el plan Free de CloudFront **no hay cargos por exceso**; un exceso sostenido durante meses puede reducir la calidad de entrega (ADR-0034, F9). La cuenta admite **como mucho tres planes Free**, sin ampliación: `dev` y `prod` usan dos.
