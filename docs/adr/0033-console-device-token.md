...
2. **Emisión: solo al final de un inicio de sesión con Google que abre la propia consola**, con la orden `atlas remote login`. Usa el cliente OAuth de ADR-0027.
   1. La consola genera un `state` y un verificador PKCE. Abre un puerto **solo en `127.0.0.1`**, solo mientras dura el intento, y lo cierra con la primera respuesta válida o cuando caduca (RFC 8252 §8.3). Después **imprime siempre la URL**, porque en la máquina del usuario no están ni `wslview` ni `xdg-open` (prueba 1), e intenta abrir el navegador sin depender de ello.
...
8. **Revocación y lista.**
   ...
   - `atlas remote logout` revoca en el servidor el token de esa consola, y solo ese, y borra su entrada local.
...
**Estado:** sigue siendo **Propuesta** hasta que el usuario la acepte. Al aceptarla se cambia la línea de estado, la fila del índice y los documentos listados en «Documentos que cambian si se acepta».
