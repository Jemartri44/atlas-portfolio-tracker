...
- **Uso único obligatorio:** el registro se crea **sin sobrescribir** (`PutParameter` sin `Overwrite`) con el `token_id` que trae el código; si ya existe, el código ya se usó: `409 console_code_used`. Que eso sea atómico ante dos canjes simultáneos **se aceptó por deducción** de la documentación de SSM (el nombre es único, sin `Overwrite` responde `ParameterAlreadyExists` y las actualizaciones concurrentes se rechazan con `TooManyUpdates`) y **se prueba contra SSM de verdad en la 018** (ADR-0033, nota del 2026-09-26; decisión de la dirección, `specs/015-api-access/questions.md` §21). Además, un código enviado otra vez se contesta **antes de revocar nada**: si su registro ya existe, `409 console_code_used` sin escribir (revisión de la PR #95, N1).
