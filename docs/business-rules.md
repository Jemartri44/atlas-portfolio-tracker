...
La ventana es de **dos meses** para valores admitidos a negociación (acciones, ETF, ETC, ETP **y participaciones de fondos**, que lo son por el art. 4.9 del RD 1082/2012 al difundir su valor liquidativo a diario; corregido el 2026-09-22, criterio #2) y de **un año** para los no admitidos, entre ellos la cripto y —por una incoherencia señalada y sin resolver— el monetario (art. 33.5 **f)** y **g)** LIRPF, **verificar**), contada **de fecha a fecha** en meses y años naturales, no en un número fijo de días (61 días no son dos meses: pregunta #14). Con aportaciones mensuales a un fondo, cualquier reembolso con pérdida de ese fondo activa la regla. Parametrizada por tipo de activo (`wash_sale_window`, `"2m"`/`"1y"`/`"<n>d"`, ADR-0013 y `data-schema.md` §8.4).
...
| `wash_sale_window{}` | cripto y monetario `"1y"`; cotizados **y fondos** `"2m"` (de fecha a fecha; `wash_sale_window_days` en días es la forma antigua aceptada). **Mapa parcial**: un tipo de activo ausente toma su valor por defecto, para que añadir un tipo nuevo no invalide la configuración ya escrita (ADR-0018) | 5.4 |
