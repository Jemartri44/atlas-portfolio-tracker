...
Son **25 tipos**. La forma exacta de cada evento (campos obligatorios, validaciones, ejemplo) se define en §6.
...
`model` (`renta` | `720` | `721`), `tax_year`, `filed_at`, `receipt_reference`, `supersedes?`, `declared`, `computed`, `ledger_fingerprint`, `notes?`
...
`movedTaxYears(events, actual, siguiente, año)` compara la base del ahorro de cada ejercicio pasado con la configuración en vigor y con la propuesta: es lo que permite que `atlas settings set` avise de que un cambio de configuración mueve un ejercicio pasado aunque no mueva ninguna ganancia realizada.
