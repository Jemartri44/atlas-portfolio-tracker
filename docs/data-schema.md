...
| `archive/ledger-<YYYY-MM-DD>-v<n>.jsonl` | Fichero anterior a cada compactación, sin tocar. `<YYYY-MM-DD>` es el día de la compactación en `Europe/Madrid` y `<n>` la **menor** `schema_version` presente en el fichero archivado; ante colisión, sufijo `-2`, `-3`… En local (`FileLedgerStore`), `archive/` es un directorio junto al libro. También archiva los bytes previos a una **restauración** (`archive/pre-restore-<fecha>.jsonl`, nunca sobrescrito, ADR-0032) y los previos a toda escritura de la sincronización que no es un añadido puro (ADR-0026, paso 6): `archive/pre-sync-<YYYY-MM-DD>T<HHMMSS>-<etag>.jsonl` antes de una sincronización o una confirmación de lo retenido que cambia líneas de sitio, `pre-join-…` antes de unirse y `pre-redownload-…` antes de volver a descargar un remoto reescrito. La fecha y la hora son de `Europe/Madrid`; `<etag>` son los 12 primeros caracteres hexadecimales del etag de los bytes archivados; si el nombre ya existe (un corte tras archivar, repetido en el mismo segundo), **solo la sincronización** (`syncDevice`) prueba `-2`, `-3`… hasta el noveno (`syncArchiveName`, `packages/domain/src/sync/archive.ts`); confirmar lo retenido, unirse y volver a descargar fallan con `archive_exists` y se repiten después. En la web, el mismo nombre es la clave `archive/<nombre>` del almacén `ledger` de IndexedDB, con `{ text, createdAt }` | Para siempre |
...
| `sync/state.json` *(por dispositivo, fuera del bucket, junto al libro local)* | **El marcador** (ADR-0026, Parte B; `packages/domain/src/sync/marker.ts`): un objeto JSON en una línea, terminado en `\n`. Campos: `sync_format` (hoy `1`); `status` (`enabled`, o `disabled` tras desactivar la sincronización de forma explícita); `synced_lines` (entero ≥ 0: cuántas líneas del principio del libro son del remoto); `synced_sha256` (SHA-256 hexadecimal, en minúsculas, de los bytes de esas líneas con su `\n`); opcionales `remote_etag`, `last_sync_at` y `disabled_at` (cadenas); y `confirmations`, la lista de lo que el usuario confirmó al resolver lo retenido, hasta que el remoto lo tiene: `{ line_sha256, duplicates, closed, confirmed_at }`, con el SHA-256 de los bytes de la línea, los `id` cuya huella repite y las presentaciones de los ejercicios cerrados que toca. Se lee de forma estricta: cualquier otra cosa es **ilegible** (`sync_marker_unreadable`). No es una simple caché: si no se puede leer, `compact` se niega; y si existe la carpeta `sync/` y falta el marcador, también se niega (ADR-0026, segunda y tercera enmiendas). **La sincronización está configurada** si existe `sync/` y el marcador no dice `disabled`; ausente o ilegible cuenta como configurada (ADR-0015, nota del cierre de la 014). Desactivar es explícito, se niega con líneas pendientes o sin marcador legible, y deja el marcador con `status: "disabled"` y lo retenido donde estaba. Para sincronizar, un marcador ausente o ilegible se reconstruye como la parte común con el remoto, pero si el libro tiene líneas propias que el remoto no tiene y no están retenidas, la sincronización para con `join_required`: unirse es siempre una elección explícita | Se sobrescribe |
| `sync/held.jsonl` *(por dispositivo, local)* | **Lo retenido** por la sincronización (ADR-0026, Parte B; `packages/domain/src/sync/held.ts`). **De solo añadir**: resolver una línea añade un registro, nunca reescribe uno. Un registro JSON por línea, con `held_format` (hoy `1`), `kind` y `at` (instante UTC), de tres clases: **`held`** retiene un miembro de una unidad, con `unit` (el SHA-256 de la primera línea de la unidad, que comparten los miembros de una pareja o una cadena), `member` y `members` (su posición y cuántos son), `origin` (`client`, `remote`, `rewrite` o `join`), `reason` (`{ code, details }`) y `line`, **la línea exacta del libro como cadena JSON**, sin tocar sus bytes. **`redo_started`** sella, antes de registrar nada, el identificador con que se rehará **esa línea**: `line_sha256` y `event_id`, un registro por línea (de una pareja, el de la anulación y el de la corrección). **`resolved`** libera la línea (`line_sha256`) con `resolution` (`confirmed`, `redone` o `discarded`); si es `redone`, lleva también `event_id`, el identificador con que se rehízo, y `replaces`, el `id` del evento retenido al que sustituye, con el que una pareja posterior de la misma cadena traduce lo que corrige. El último registro de una línea decide su estado. Se lee de forma estricta (`sync_held_unreadable`). `atlas backup` lo copia **entero**, con su historial, junto a la copia del libro como `ledger-<YYYY-MM-DD>.held.jsonl`, verificado, y solo si hay algo retenido sin resolver; si no, lo dice | Para siempre (de solo añadir) |
| `sync/discarded.jsonl` *(por dispositivo, local)* | **Lo descartado** por el usuario y lo sustituido al rehacer. **De solo añadir.** Un registro JSON por línea: `discarded_format` (hoy `1`), `at`, `decision`, `reason` (`{ code }`, `discarded_by_user` o `redone`), `replaced_by` (si se rehízo: el identificador sellado que la sustituyó) y `line`, la línea exacta como cadena JSON. **`decision` nombra la decisión, no los bytes**: el SHA-256 de la resolución, la unidad, la hora en que se retuvo y el hash de la línea. Repetir la misma decisión tras un corte no la anota dos veces; los mismos bytes retenidos otra vez y descartados otra vez sí. Los registros escritos antes de este campo no lo llevan | Para siempre (de solo añadir) |
| Claves `sync:state`, `sync:held` y `sync:discarded` *(web, IndexedDB)* | Los mismos tres ficheros en la web, **como texto**, byte a byte con el formato de arriba, en el almacén `ledger` junto al libro (`current`) **sin subir `DB_VERSION`** (`packages/adapters/src/ledger-store/browser/sync-store.ts`). El libro, lo retenido, lo descartado, el marcador y el archivo de una reescritura se comparan y se escriben en **una sola** transacción de lectura y escritura con `durability: "strict"` (§5). La sincronización está configurada si existe alguna de las tres claves y el marcador no dice `disabled` | Como sus ficheros |
...
| `~/.config/atlas/credentials.json` *(fuera de la carpeta del libro y del bucket)* | Los tokens de dispositivo de la consola frente a la API (ADR-0033): **un mapa por `device_id`**, y cada entrada lleva su `origin` y su token, porque es **un token por dispositivo y por carpeta** (ADR-0033, punto 6; dos carpetas contra el mismo origen tienen dos entradas); o en `$XDG_CONFIG_HOME/atlas/`. Cada entrada guarda además, como pista, la ruta absoluta de la carpeta desde la que se inició sesión. Qué entrada es la de una carpeta lo dice su `sync/remote.json` (`{ format: 1, origin, device_id }`, lectura estricta), que **no** escribe `atlas remote login` —iniciar sesión solo escribe aquí y nunca configura la sincronización— sino inicializar la sincronización remota o unirse a ella, en la misma secuencia que el marcador (decidido por la dirección el 2026-09-25 para la feature 015, `docs/prompts/015-api-access.md` §7 P7 y P16; sin implementar todavía). *(Corregido el 2026-09-25: decía «uno por origen».)* Hermano de `secrets.json` y con **sus mismas reglas**: permisos `600` o no se usa, la consola se niega si esta carpeta y la del libro están una dentro de la otra, y **ni la web ni ninguna copia, exportación o sincronización lo leen**. La diferencia: **lo escribe la consola**, solo al iniciar y al cerrar sesión (`atlas remote login`, `atlas remote logout`), de forma atómica y creado ya con `600`. El token nunca va en un argumento, una variable de entorno, una URL ni la salida de la consola. Formato del token, en `docs/api.md`. **El registro del servidor no vive en el bucket**: es un `SecureString` de SSM por token, con solo el hash (`/atlas/<entorno>/device-tokens/<id>`), para que ninguna copia ni restauración del bucket lo toque | Hasta que caduca o se revoca; lo sustituye el siguiente inicio de sesión |
...
**Operaciones sobre líneas crudas del puerto `LedgerStore` (ADR-0026, Ronda 8).** `append` y `replace` reciben `LedgerEvent[]` ya migrados en memoria y los vuelven a serializar (`encodeLine`); eso hace imposible que la réplica de un dispositivo sincronizado sea idéntica byte a byte al remoto. El puerto gana **dos operaciones que escriben los bytes tal cual, sin volver a serializarlos**: `appendLines`, que añade líneas crudas al final, y `replaceLines`, que reemplaza el contenido por líneas crudas archivando antes el original — con los mismos etag, archivo y rechazos que `append` y `replace` (`packages/domain/src/ports/ledger-store.ts`). Antes de escribir un byte, cada línea se comprueba como la leerá el cargador (`rawLinesText`, `packages/domain/src/schema/raw-lines.ts`), y una línea con un salto de línea o un retorno de carro (`"\n"` o `"\r"`) dentro se rechaza con `raw_line_break`: un libro escrito a mano con finales de línea de Windows no se sincroniza ni se restaura hasta convertirlo a finales LF, y el mensaje de la consola da la orden exacta, con una copia previa del libro que nunca se sobrescribe (decisiones D-Q10 y D-Q18 de la 014). Hoy las cumplen **tres** adaptadores (memoria, fichero y navegador), que pasan los mismos tests de contrato; el de S3 es de la feature 015 y pasará los mismos. **La sincronización y la restauración usan solo esas** (ADR-0026, Parte A; ADR-0032): `replace` seguiría siendo un `compact` sin resellar las presentaciones si se usara para restaurar con una `schema_version` más nueva.
...
- **Lo cubre todo lo que se escribe en la carpeta**: el libro, `archive/`, `drafts/`, `reference/ecb/` y `sync/`. Se toma antes de comparar el etag y se suelta después de renombrar; una sincronización o una resolución de lo retenido comparan y escriben todo bajo **una sola** toma (`FileLedgerStore.underLock`), porque el cerrojo no es reentrante. Al empezar cada orden, los temporales huérfanos se barren **solo** tomando el cerrojo: los del libro (`ledger.jsonl.tmp-<pid>-<hora>`) y, desde la 014, los de `sync/` (`state.json`, `held.jsonl` y `discarded.jsonl` con `.tmp-<pid>-<hora>`; decisión D-Q19).
...
**IndexedDB: comparar y escribir en una sola transacción.** El almacén de la web no vive en una carpeta: su primitiva es `LedgerBlob.update` (`packages/adapters/src/ledger-store/blob.ts`), que lee, compara el etag y escribe en **una sola** transacción de lectura y escritura. Toda lectura seguida de escritura del libro pasa por ella: `append`, `replace`, la exportación (que lee el texto y anota la fecha en la misma transacción) y la importación (que compara con el libro que había al preguntar y **se niega si cambió**). La fecha de la última exportación vive aparte del texto (`current:meta`), y una importación no hereda la del libro que sustituye. **La sincronización** (feature 014) escribe el libro, las claves `sync:*` (§1) y el archivo de una reescritura en **una sola** transacción con `durability: "strict"`, que abre su propio almacén (`BrowserSyncStore`) y no `LedgerBlob.update`: ningún navegador asegura en disco por defecto, y lo retenido tiene que estar en disco antes de que el libro pierda la línea (ADR-0026, nota del cierre de la 014). La base está en `DB_VERSION` 2, con el almacén `drafts`; si otra pestaña con una versión anterior bloquea la subida, se dice así, no «el navegador no permite guardar datos».
...
Un `settings_changed` que reinterpreta el pasado (p. ej. un cambio de `fiscal_date_rule`) puede dejar eventos históricos inválidos bajo la regla nueva. Es el **único** evento que se admite registrar aun así, con confirmación explícita que lista los eventos afectados (los hechos no cambian; cambia su interpretación, y no hay nada que rectificar antes). Las consultas de solo lectura proyectan entonces en modo degradado (`collectErrors`) con un aviso en cabecera; las mutaciones se niegan así: `recordEvent` se niega mientras queden eventos inválidos anteriores, salvo con un `settings_changed`; `correctEvent` y `reverseEvent` solo se niegan por los inválidos que ellos mismos crean (*challenge* 2026-08-31, hallazgo 2; feature 004; regla precisada en la nota del 2026-09-25 de ADR-0015).
