...
- **017 — Infraestructura como código** (ADR-0028, ADR-0033 y **ADR-0034**: cuenta compartida). Módulos de Terraform, el prefijo `/atlas/<entorno>/device-tokens/` con sus permisos (sin `DeleteParameter` ni `LabelParameterVersion`), la política de origen que reenvía a `/api/*` `x-atlas-device-token`, `Sec-Fetch-Site` y `Origin` (`docs/api.md` §8), *bootstrap* **por entorno en la cuenta compartida**, reaplicado a mano en cada cambio (primero `dev`, después `prod`), con los límites de permisos de cada entorno y su prueba con el simulador de políticas de IAM, la política del bucket de datos, las etiquetas y el presupuesto filtrado por `project=atlas`, la CloudFront Function de la CSP, el guion del plan de tarifa plana y los flujos de GitHub Actions con OIDC; solo `fmt` y `validate`.
