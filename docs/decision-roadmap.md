...
**017 — Infraestructura.** Además de lo de su entrada: la prueba de los límites de permisos con el simulador de políticas de IAM, **como parte de su definición de terminado**; el rol de `plan` sin `ReadOnlyAccess`; localizar la PR del proveedor de Terraform para los planes de tarifa plana y enlazarla en ADR-0028; los permisos de KMS para `aws/ssm`; y que la función de la CSP respete la `sandbox` de la página del código. Instalar Terraform en local es decisión del usuario (herramientas).
