...
**Propuesta y revisada el 2026-09-25, pendiente de que el usuario la acepte: ADR-0033**, cómo se autentica la consola frente a la API, que ADR-0027 dejaba fuera. Propone un token de dispositivo propio, emitido solo por la consola (`atlas remote login`) al final de un inicio de sesión con Google (*loopback* con PKCE y una variante manual) y nunca desde la web, válido 90 días, guardado en `~/.config/atlas/credentials.json` y registrado en SSM. Mientras no se acepte, la 014 puede fijar en `docs/api.md` la cabecera y las rutas sin implementarlas; lo que cambia al aceptarla está listado en la propia ADR.
