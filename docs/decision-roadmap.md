...
Escrita para que una dirección nueva, sin memoria, retome el plan desde el repositorio. Complementa el plan por etapas de arriba, no lo repite: aquí van **las dependencias, lo que cada etapa hereda y lo que no tenía dueño**. Auditada contra `develop` (`3be2480`) y contra la PR #84, abierta, del cierre documental de la 014, que añade a la entrada de la 015 la lista detallada de lo que la 014 le deja.
...
**015 — API y acceso.**
- *Hereda de la 014* (fuente: `specs/014-ledger-sync-core/questions.md` §12-§15 y `deferred/README.md`; lista detallada en la entrada de la 015, arriba, cuando se fusione la PR #84):
  - **requisito duro**: P2 y P3 de la web antes de permitir configurar la sincronización en la web, con el parche `deferred/p2-p3-web.patch`;
  - la ligadura del dispositivo de la web con su sesión (`docs/api.md` §5.4);
  - las órdenes de administración contra el remoto: `compact`, restaurar y olvidar un dispositivo (su negativa ya es `rewritePermission`);
  - el rehacer que no se puede terminar tras registrar a mano media pareja, `redo_waits_for_pair` limitado a la unidad, los mensajes de desactivar y de `join_required`, y los huecos de la propiedad «ninguna línea se pierde».
...
- *Antes*: el usuario hace C1-C19 y contesta las preguntas de ADR-0034. Con la respuesta a C2, la dirección decide si `prod` lleva una clave KMS propia; con la de C11, si `dev` es permanente o bajo demanda.
- *Tareas del usuario*: la verificación en dos pasos de Google, los clientes OAuth de `dev` y `prod`, la verificación del remitente en SES, los valores de SSM (lista permitida, destinatario, interruptor de importes), la activación de la etiqueta `project` y los subdominios en `terraform.tfvars`.
- *Dentro*: extremo a extremo con datos sintéticos; el ensayo de restauración; destruir `dev` y levantarlo desde cero (ADR-0032, nota del 2026-09-25); y, al mes, contrastar el coste etiquetado con Cost Explorer (ADR-0034, Consecuencias).
...
- **`atlas backup` se amplía a `documents/` e `imports/`** (ADR-0032, capa 4). Ninguna feature lo tenía; sin ello, un evento corporativo pierde su fuente documental obligatoria si se pierde la cuenta. Candidata: la 015, junto a restaurar.
