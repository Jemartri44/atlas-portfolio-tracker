...
- Tareas del usuario, con procedimiento escrito: **las comprobaciones de solo lectura C1-C19 de ADR-0034** sobre su cuenta compartida (con las respuestas fuera del repositorio salvo sí, no o un número), el MFA del *root* de esa cuenta, la activación de la etiqueta `project` en Billing, la verificación en dos pasos de su cuenta de Google, la petición de aumento de la cuota de concurrencia si hace falta, los clientes OAuth de `dev` y `prod`, la verificación de la dirección en SES, las claves en SSM y la lista permitida.
- **018 — Despliegue de `dev`** en la cuenta compartida, con datos sintéticos: extremo a extremo (acceso, dos dispositivos sincronizando, tareas), el ensayo de restauración y, en lugar del de pérdida de la cuenta, **destruir `dev` y levantarlo desde cero** (ADR-0032, nota del 2026-09-25).
