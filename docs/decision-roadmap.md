...
- **017 — Infraestructura como código** (ADR-0028, ADR-0033). Módulos de Terraform, el prefijo `/atlas/<entorno>/device-tokens/` con sus permisos (sin `DeleteParameter` ni `LabelParameterVersion`), la política de origen que reenvía `x-atlas-device-token`, *bootstrap*, la CloudFront Function de la CSP, el guion del plan de tarifa plana y los flujos de GitHub Actions con OIDC; solo `fmt` y `validate`.
