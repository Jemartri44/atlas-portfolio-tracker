...
| 13 | Fusión o canje con **compensación en efectivo** ("1 nueva por 2 antiguas más 3 € por antigua"): ¿el efectivo tributa como ganancia patrimonial en el ejercicio del canje? ¿Con qué coste (proporcional a las antiguas entregadas, cero, o reduce el coste de las nuevas)? ¿Y los picos liquidados en efectivo? | `data-schema.md` §6.5, primitiva `forced_sale` | Venta parcial de las antiguas con cantidad y precio fijados a mano; los picos como venta |
| 14 | Regla de recompra: ¿la ventana de dos meses / un año se cuenta **de fecha a fecha** en meses y años naturales (venta 01-07, ventana posterior hasta 01-09 inclusive)? ¿Qué pasa cuando el mes de destino no tiene ese día (31-01 + 1 mes)? | ADR-0013, `wash_sale_window` | De fecha a fecha; fin de mes al último día |
| 15 | Pérdida diferida por recompra cuyos lotes recomprados se **traspasan o canjean** antes de liberarse: ¿el diferimiento sigue a los lotes descendientes (que conservan antigüedad y coste) y se libera al transmitirse estos? | `data-schema.md` §8.4, motor fiscal (Fase 5) | Sí: viaja con el linaje (`source_lot_id`) |
| 16 | Deducción por doble imposición de dividendos extranjeros: ¿limitada al **tipo del convenio** con el país del pagador (el exceso se reclama al fisco de origen) y a la cuota española correspondiente? | `dividend.source_country`, Fase 5 | Sí a ambas; registrar país y retención íntegra |
