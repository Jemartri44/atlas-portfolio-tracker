...
> Quinta versión, 2026-09-18; **estado (§5, §6 y §8) puesto al día el 2026-09-23**, al cerrarse la Fase 5. El modelo de trabajo no ha cambiado. Léela entera antes de hablar con el usuario. Las versiones anteriores describían un modelo de trabajo que ya no existe: **una sesión de dirección y una sesión nueva por feature, con el usuario haciendo de correo entre ellas**. Desde el 2026-09-18 hay **una sola sesión permanente** que dirige y orquesta. Esta versión recoge la corrección que el usuario hizo el mismo día: **la dirección orquesta, no ejecuta** (§1, §3, §4).
...
**En `develop`:** Fases 1, 2, 3 y **5** completas y fusionadas, más la aplicación web entera. Monorepo npm workspaces con `packages/domain` (dinero decimal sobre `big.js` vendorizada, **25 tipos de evento**, FIFO global, eventos corporativos con cinco primitivas, tesis, `compact`, verificación profunda, generador sintético con *golden file* congelado, precios manuales, pesos del núcleo, calculadora de aportación, costes, proyección a una fecha, y el motor y la salida fiscales colgando de un punto de entrada propio, `@atlas/domain/fiscal`), `packages/adapters`, `apps/cli` (`atlas`, en español) y `apps/web`. Cobertura del 100 % de líneas y ramas en el dominio, bloqueante en CI. **24 ADRs**, todos aceptados; constitución **1.5.0**, sin enmendar desde el 2026-09-18. Van **67 PRs**: 66 fusionadas y la del cierre documental de la 010, abierta.
...
**En vuelo:** ningún código. Solo la PR de cierre documental de la 010, que deja **`docs/pendientes-post-010.md`**: las doce anotaciones no bloqueantes de su revisión adversarial, comprobadas una a una contra el código fusionado, con **nueve todavía vivas** y, de cada una, lo que cuesta y lo que se pierde. Es el material para decidir la próxima ronda.
...
| Lo que la revisión de la 010 dejó abierto | **Sin decidir, y es lo primero que tienes encima.** Nueve anotaciones vivas en `docs/pendientes-post-010.md`, con su coste y su consecuencia. Dos merecen mirarse antes que las demás: la lectura alternativa que lanza `tax_year_unsupported` y deja **inobtenible** el informe de un ejercicio soportado, y el aviso de ejercicio cerrado que puede callarse del todo con libro inválido y cambio fechado fuera del ejercicio |
...
2. Lee entero **`docs/pendientes-post-010.md`** y **decide la próxima ronda**: qué entra, qué se descarta y qué necesita ADR. Son nueve, ninguna bloqueante, y dos cambian lo que el usuario ve.
3. Con la Fase 5 cerrada, toca el ***challenge* de decisiones** antes de abrir fase nueva: `docs/prompts/review-challenge-decisions.md` es reutilizable y los tres anteriores encontraron 25 hallazgos.
4. Rondas de pulido visual cuando el usuario las pida, con el verificador midiendo las tres dimensiones de §4 y **mirando tú las capturas**.
