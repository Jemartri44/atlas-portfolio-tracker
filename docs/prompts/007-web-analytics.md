...
- **Costes**: `costSummary`, con las comisiones que suman al coste de adquisición y, **en bloque aparte**, las comisiones sueltas, etiquetadas como que no forman parte de ese coste.
  *Corrección (Q9 de la 007): este prompt decía "separadas de las que no son deducibles" dando por hecho que `costSummary` veía las dos mitades. **No las ve**: solo mira `buy`, `sell` y las ventas forzosas —justo las deducibles— y no mira `standalone_fee` en absoluto, así que no había nada que separar. Lo destapó el implementador. Se añade la mitad que falta al dominio, **sin clasificar por tipo**, porque el campo que distingue custodia de conectividad (`fee_kind`) lo añade la feature 008 (ADR-0021) y aún no existe. Efecto secundario bueno: es la primera vista donde el usuario ve lo que le cuesta la custodia.*
...
## 7. Respuestas a las preguntas de la 007

Once preguntas del implementador tras leer el prompt y medir el libro. Se registran aquí porque las respuestas forman parte del encargo.

- **(Q1) Huecos en las series.** El eje X son **las fechas de valoración del libro**, no una rejilla fija; el hueco es **por serie**; y **un punto solo existe cuando todos sus componentes tienen precio**. Nunca se dibuja un total `partial`: es la suma de lo que sí tiene precio, es menor que la realidad y **es indistinguible de una caída**, así que dibujarlo inventaría una pérdida que no ha ocurrido. La pantalla dice cuántos puntos hay y por qué faltan los demás. *Medición que lo motiva: de 29 cierres mensuales del libro sintético, solo **6** son completos para el núcleo, **3** para el cubo y **2** para los tres bloques a la vez. Una línea única del patrimonio tendría 2 puntos de 29. Eso no es un defecto de la gráfica: es información sobre lo poco que se valora, y el usuario tiene que verla.*
- **(Q2) El aviso de traspaso vencido** vive en una proyección nueva que envuelve `pendingTransfers` (patrón `bucketTheses`→`theses`). Código `transfer_overdue`; operador **`>`**, porque el parámetro dice "más de N días" y el día del plazo aún está en plazo; y la etapa `redeemed` **sí** cuenta, que es el estado en que el dinero ya salió del fondo de origen y no ha llegado a ninguna parte.
- **(Q3) `--date` se añade a `atlas transfer pending` y también a `atlas order list`**, que tiene el mismo defecto y además falsea `days_open` a 0 con `--all`. Arreglar la mitad de un defecto de clase es como sobreviven las inconsistencias.
- **(Q4) La composición de efectos de un evento corporativo baja al dominio**, y **no lleva ADR**: mover una regla de negocio de una aplicación al dominio *es* la regla (ADR-0007, decisión (h) de la 006), no una excepción que justificar. Un ADR se escribe al decidir, no al aplicar lo decidido. El caso lo confirma: repartir los picos de un contrasplit con `posición − ⌊posición⌋` determina cuánto se vende forzosamente, o sea cuánta ganancia se realiza.
- **(Q5) `components/chart/axis.ts` se autoriza** a importar `format/money.ts`, con motivo escrito y con el test que renderiza en modo privacidad. El criterio de fondo: **el eje de una gráfica es un importe a todos los efectos**, así que con la privacidad activa no puede haber cifras absolutas en él. Si eso deja la gráfica coja en privado, queda coja: es lo que el modo privacidad promete.
- **(Q6) El presupuesto del *bundle* pasa a dos cifras**, arranque y total, y lo primero es arreglar el comprobador, que dice medir el arranque y suma todo `dist`. Techos: **arranque ≤ 80 KB gzip**, **total ≤ 150 KB gzip**, con uPlot solo en fragmentos perezosos. Al terminar se fija el total en lo medido, no en el techo.
- **(Q7) Curva agregada del cubo frente al índice**, con la tabla por tesis debajo. Nueve series en 400 px no se leen y el color no puede ser el único portador de significado. **Gana `docs/specification.md` §6.2 sobre este prompt**, y queda el precedente escrito —ya pasó en la 006 con el modo privacidad—: **cuando el prompt y la especificación se contradigan, manda la especificación.** El prompt es un encargo; la especificación es el producto.
- **(Q8) El techo de 250 líneas es para `.ts/.tsx`.** El CSS lleva razón escrita (partir `components.css` dispersaría la cascada, que es donde nacieron los defectos que solo se vieron en el navegador). De los seis ficheros que ya se pasan, `forms/specs.ts` y `view-models/settings.ts` son tablas de datos y llevan razón; `registrar/EventForm.tsx` y `movimientos/detail.tsx` se parten. Un test lo vigila.
- **(Q9) Corregido arriba, en §3.1.** El prompt pedía separar dos mitades cuando el dominio solo tenía una.
- **(Q10) El "valor por defecto" de la fecha fiscal se arregla.** Un control que promete restaurar el valor por defecto y no hace nada es peor que no tenerlo, porque el usuario cree que ha cambiado algo. ADR-0018 legitima la semántica (los mapas son parciales y el tipo ausente toma el valor documentado) y ADR-0015 ya protege el caso peligroso al registrar el `settings_changed`.
- **(Q11) Las tres filas del inventario de errores entran**, incluida la traducción de los diez hallazgos de integridad: la aplicación está en español para alguien que no tiene por qué leer inglés, y una comprobación que informa en inglés es inútil justo cuando hace falta.
