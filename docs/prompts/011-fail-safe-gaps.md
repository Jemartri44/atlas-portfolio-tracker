...
>
> **Requisito previo, ya cumplido.** La PR de cierre documental de la 010 (**#67**) está **fusionada en `develop`** desde el 2026-09-23, y con ella llegan las dos cosas de las que depende esta feature: **`docs/pendientes-post-010.md`**, el inventario del que sale casi todo el alcance, y la **ADR-0024 en estado Aceptada**, que gobierna cuatro de los bloques y lleva dentro el criterio de **qué es nota y qué es dato**. Los dos se leen de `develop`, sin rodeos.
>
...
1. Worktree separado, rama desde `develop` **actualizado**:
   ...
   ```
2. Spec Kit en `specs/011-fail-safe-gaps/` (español, identificadores en inglés). **Para después de `spec.md` y `plan.md`**, con tus preguntas en `specs/011-fail-safe-gaps/questions.md`, y espera el visto bueno antes de escribir código: **quien contesta es la dirección**, no lo resuelvas tú por falta de respuesta. En las diez features anteriores ese alto destapó las preguntas buenas, y en esta hay **dos** cosas que tienen que llegar **antes de que toques una línea**, no dentro del primer commit: las dos premisas del bloque 3 y el mapa de guardias del bloque 4.
...
- **`npm run lint` verde antes de cada commit y otra vez como último paso antes de entregar.** Empujas **tu rama y solo tu rama**, para abrir la PR de §2.4: nunca a `develop` ni a `main`, y **nunca fusionas nada**.
...
- **Vigila el coste.** El hecho del ejercicio cerrado vive en el trozo de arranque de la web (`filings/touched.ts`, 0,4 KB) precisamente porque meter ahí la cifra costó 4,9 KB y arrastró `tax/` al arranque. **No deshagas ese reparto**: el desenlace nuevo se emite donde hoy se emite la cifra. Y mide: sobre `develop` (medido el 2026-09-23 con `npm run build`) quedan **0,6 KB de margen de arranque** —**72,9** contra un techo de 73,5— y **1,0 KB de total** —**235,0** contra 236,0—. Si no cabe, **para y avisa**: el techo es de la dirección, con su motivo escrito en `check-bundle.mjs`, y no se sube por iniciativa del implementador ni se esconde con una importación dinámica.
...
- **Decisión de formato de la dirección: una celda vacía deja de ser expresable.** Cada variante declara **su** dirección de riesgo, con los **cuatro valores que ya existen** en `RiskDirection`. El silencio deja de ser un estado y pasa a ser una afirmación que alguien firma. Con eso, el mapa `DOCUMENT_SILENT` del test —cuatro exenciones escondidas en código— desaparece.
...
- **El presupuesto del paquete web, medido**: arranque y total dentro de sus techos (73,5 y 236,0 KB gzip), partiendo de **72,9 y 235,0** en `develop`, con lo medido escrito. Si no cabe, se para y se avisa.
