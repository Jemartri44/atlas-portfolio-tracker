...
1. Worktree separado, rama desde `develop` **actualizado**:
   ...
   ```
2. Spec Kit en `specs/011-fail-safe-gaps/` (español, identificadores en inglés). **Para después de `spec.md` y `plan.md`**, con tus preguntas en `specs/011-fail-safe-gaps/questions.md`, y espera el visto bueno antes de escribir código. En las diez features anteriores ese alto destapó las preguntas buenas, y en esta hay **tres** cosas que tienen que llegar antes de tocar nada: las dos premisas del bloque 3, el mapa de guardias del bloque 4 y la celda de `2:other` del bloque 7.
...
- **Las cuatro celdas que hoy el documento calla, decididas por la dirección.** La regla que resuelve tres de ellas es mecánica: **una ventana de recompra más larga es siempre la dirección conservadora**, porque bloquea más pérdidas en el ejercicio corriente, difiere más pérdida y hace pagar más ahora.
  - **`2:listed_1y` → conservadora.**
  - **`2:fund_1y` → conservadora**, que además es lo que el catálogo ya declara en el código; la celda muda solo existe en el documento.
  - **`2:crypto` → conservadora**: un año por prudencia sin norma expresa es elegir la ventana larga.
  - **`2:other` → «lectura literal, sin alternativa en disputa»**, por ser un año para los no admitidos a negociación lo que dice la letra g) del art. 33.5, sin lectura contraria que defender.
- **Y la cuarta no encaja: es una pregunta abierta, no una instrucción.** La redacción de este prompt ya la ha comprobado y lo deja escrito para que no se fuerce: en el catálogo, **`2:other` no es «un año para los no admitidos»**. `windowCriterion` (`packages/domain/src/tax/lines.ts`) devuelve `2:other` para **cualquier ventana que no sea dos meses ni un año, en cualquier tipo de activo** —es decir, un número de días arbitrario—, mientras que el año de un valor no admitido cae en `2:fund_1y` o en `2:listed_1y`. Además, el catálogo declara hoy `2:other` con riesgo **`both`**, y «lectura literal, sin alternativa en disputa» no es ninguno de los cuatro valores de `RiskDirection`. **Para y pregunta antes de escribir esa celda**: cambiar lo que el catálogo declara de `2:other` sería cambiar un criterio, y eso no es tuyo. La respuesta irá a §7.
...
- **Resolver cualquier criterio fiscal en disputa.** Seis siguen sin resolver y así se quedan: resolverlos sería sustituir una lectura no verificada por otra. Cambiar lo que el catálogo declara de `2:other` (bloque 7) es cambiar un criterio: se pregunta, no se hace.
...
- **Las tres comprobaciones previas, escritas y reportadas antes de tocar lo que dependa de ellas**: las dos premisas del bloque 3, el mapa de guardias del bloque 4 y la celda de `2:other` del bloque 7.
...
- **`docs/` sin cambios salvo `docs/fiscal-questions.md`** (bloque 7), y `specs/011-fail-safe-gaps/questions.md` con lo preguntado, las tres comprobaciones previas, cómo viste fallar cada test, lo que decidas en el plan y **la lista de documentos que la dirección tendrá que actualizar** (como mínimo: `docs/data-schema.md` §4 —la tupla de la huella cambia—, §5 y §7; `docs/business-rules.md` si cambia un mensaje que ella describe; y `docs/prompts/README.md`).
...
- **(k) `docs/fiscal-questions.md` pasa a una fila por variante, legible por máquina, con permiso expreso de la dirección**, y **una celda vacía deja de ser expresable**: cada variante declara una dirección de riesgo o el valor explícito «lectura literal, sin alternativa en disputa». Desde que los criterios firmes se separan de los dudosos, la certeza decide si al usuario se le dice «nadie sabe cómo se lee esto» o «esto está resuelto». **Analizar prosa española queda descartado**, y **inventar** una certeza o un riesgo que el documento no dice, también.
