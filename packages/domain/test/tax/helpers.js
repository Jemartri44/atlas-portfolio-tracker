...
export const text = (money) => (money === undefined ? "—" : money.amount.toString());
/**
 ...
 */
export const taxBuilder = (settings = DEFAULT_SETTINGS) => {
...
export const reportOf = (events, year, today = "2035-01-01") => taxYear(events, year, { today });
export const lineOf = (report, eventId, account) => {
 ...
 */
export const rowsAddUpToTheEngine = (events, year, today = "2035-01-01") => {
  ...
  const sumOf = (predicate) =>
    boxes.entries
      ...
      .reduce((total, entry) => total.add(entry.exact_eur), zero);
  const rows = sumOf(
    (entry) =>
      entry.row !== undefined &&
  ...
  const foreign = chain.walk.outcomes
    .filter((outcome) => chain.state.gains[outcome.gain_index].year === year)
    .flatMap((outcome) => outcome.foreign_released)
    .filter(
      (release) =>
        categoryOf(chain.state, chain.state.gains[release.origin].asset_id) === "capital_gain",
    )
