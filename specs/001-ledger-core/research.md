...
| R4 | ULID propio: 48 bits de tiempo (`clock.now()`) + 80 bits aleatorios, monótono en proceso incrementando la parte aleatoria si el milisegundo se repite. La aleatoriedad llega por un puerto mínimo `RandomSource` (`(bytes) => void`) que `adapters` implementa con `crypto.getRandomValues`; el dominio no toca ningún global (ajustado al implementar: el tipo `crypto` no existe en `lib: ES2022` sin DOM) | Prompt §3.3; mantiene el dominio isomorfo y sin globales | `globalThis.crypto` directo en el dominio (obliga a declarar el global y choca con `@types/node` en los consumidores); puerto `IdGenerator` |
...
| R16 | Versiones de desarrollo instaladas: `typescript` 7.0.2, `@biomejs/biome` 2.5.11, `vitest` y `@vitest/coverage-v8` 4.1.11, `fast-check` 4.9.0, `@types/node` 22.x, fijadas por `package-lock.json`. `vitest` 4 exige `vite` como *peer*: entra en el lockfile (está en el presupuesto para la web) sin declararse en `package.json` | Presupuesto cerrado pero sin versión fijada en `docs/dependencies.md` | — |
