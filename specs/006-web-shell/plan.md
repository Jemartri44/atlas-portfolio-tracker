...
**D11 — Las tablas densas se reorganizan, no se desplazan.** En móvil, cada fila del libro es una tarjeta de dos alturas (línea 1: fecha, tipo y estado; línea 2: cuenta/activo e importe alineado a la derecha) con toda la tarjeta como objetivo táctil; en pantalla ancha, la misma información es una `<table>` nativa con cabeceras. Una sola fuente de datos (`view-models/movements.ts`), dos presentaciones por CSS. Cero desplazamiento horizontal a 360 px, que es criterio de aceptación (SC-001).

> **Corrección de la segunda revisión (2026-09-18).** Este punto decía "a partir de 768 px" y ahí la tabla **no cabe**: a 768 px el rail se lleva 216 px y la tabla densa pide 659 px de los 505 que quedan, así que la página se desplazaba 24 px de lado (medido en el navegador; la barra inferior y el rail siguen conmutando a 768 px, que es A10). La tabla entra a **1024 px**, donde sobra ancho; entre 768 y 1023 se ven las tarjetas, que es la misma información en una columna de 505 px. Es la única anchura del armazón que no coincide con el punto de ruptura único, y está escrita con su medida en `components.css`.
...
**D16 — Tests sin entorno de DOM** (decisión (k)). Lo que se prueba: formato, orden de avisos, mapeo y filtrado de movimientos, especificaciones de formulario contra el esquema, contrato del adaptador (con los tests ya escritos), catálogo de mensajes, y los flujos completos **a nivel de acciones** (abrir un libro de prueba → proyectar → construir borrador → vista previa → escribir → releer, comprobando que el fichero gana exactamente las líneas esperadas y que las anteriores no cambian). Lo que se verifica a mano y se anota (criterio §5 del prompt): 360 px sin desplazamiento, objetivos táctiles, la aplicación abriendo el `ledger.jsonl` del *golden* y registrando una compra.

> **Corrección de la revisión (2026-09-18).** Esta lista afirmaba de más: el **enmascarado** y la **selección de vía de almacenamiento** no estaban probados. Del enmascarado solo se comprobaba que la máscara es una constante, y `ledger/source.ts` no lo importaba ningún test, así que el aviso de exportación —la única red de seguridad en el teléfono, decisión (l)— podía dejar de saltar sin que nada fallara. Cubiertos ahora: `amountDisplay` y `privacyFromPreference` en `test/format.test.ts`, y `daysSinceExport`/`exportIsOverdue`/`sourceLabel` en `test/source.test.ts`, con las cinco mutaciones de la revisión comprobadas una por una.
...
- `npm run lint`, `npm run typecheck`, `npm test` (837 pruebas) y `npm run build` en verde; `npm run clean && npm run build` desde cero.

  > **Corrección de la revisión (2026-09-18).** Al cerrar la feature el `lint` **no** estaba en verde: `apps/web/test/view-models.test.ts` arrastraba un `lint/correctness/noUnsafeOptionalChaining` desde cinco commits antes de HEAD, así que la CI no pasaba. Corregido, y comprobado otra vez al final de la revisión. Las cifras de esta sección son las de después de las correcciones; el recuento de pruebas subió de 812 a 837.
...
- Seis reglas de arquitectura nuevas: la puerta de `Amount`, la prohibición del barril de `@atlas/adapters` y de `node:*`, la ausencia de directivas `use:`, la de orígenes ajenos en las fuentes y, desde la revisión, las **dos direcciones entre el CSS y el marcado** (ninguna clase del CSS sin elemento, ningún elemento con una clase que nadie declara). **Se ha comprobado que fallan** introduciendo una violación a propósito.
- El test anti-deriva de mensajes: todos los códigos del dominio traducidos en las **dos** interfaces.
- El `build` comprueba sobre el resultado que no hay `node:`, ni URL ajena, ni **estilo en línea** (`style=` o `<style>`, que la CSP de producción bloquea), ni exceso de presupuesto. El filtro de URL ya no exige que el *host* tenga un punto —así se colaba un `http://localhost:9999/beacon`—: los dos centinelas inertes del router están nombrados uno a uno.
