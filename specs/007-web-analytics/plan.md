...
Las de partida están en [`research.md`](research.md). Todas las de abajo están medidas en este worktree: las del dominio con Node, las del navegador con el Chromium del entorno conducido por el protocolo DevTools desde el *scratchpad* (nada de Playwright en el repositorio).

### Coste de calcular

| Qué | Antes | Después |
|---|---|---|
| Proyección del libro sintético, en caliente | 1,95 ms | igual (no se ha tocado el motor) |
| Serie del patrimonio, fechas por defecto (8 puntos) | — | **34,4 ms** en frío, 15,3 ms en caliente |
| Serie del patrimonio, rejilla mensual forzada (29 puntos) | — | **37,8 ms** |
| Serie del cubo frente al índice (8 puntos) | — | **11,6 ms** |

La serie más larga que el libro sintético produce tiene **8 puntos**, porque son las fechas en las que hay valoración. Forzar 29 puntos mensuales cuesta 37,8 ms, así que el tope de 120 puntos deja margen de sobra para veinte años.

### Interacción, medida en el navegador (400×890, DPR 3)

De la acción hasta que el DOM muestra la respuesta nueva (dos `requestAnimationFrame`):

| Interacción | Núcleo | Cubo |
|---|---|---|
| Cambiar la fecha | **31,2 ms** | 2,1 ms |
| Cambiar el rango de una gráfica | 18,9 ms | 14,7 ms |
| Activar el modo privacidad | 14,3 ms | 13,8 ms |
| Entrar en la pantalla | 4,2 ms | 4,5 ms |

Todo muy por debajo de los 100 ms de SC-004, así que **ninguna operación tiene que anunciarse ni cancelarse**. La más cara es cambiar la fecha en Núcleo, que reproyecta el libro y recalcula pesos, aportación, costes y la serie entera.

### Arranque con la CPU frenada

| CPU | Pantalla final | Cambios de ruta intermedios |
|---|---|---|
| ×1 | `/` (Resumen) | ninguno |
| ×4 | `/` (Resumen) | ninguno |
| ×10 | `/` (Resumen) | ninguno |

La carrera que la 006 arregló (S3 de sus notas) sigue arreglada con dos pantallas más y una dependencia más.
...
| Paquete | Ficheros | Líneas |
|---|---|---|
| `packages/domain/src` | 73 (+2) | 12.331 (+758) |
| `apps/cli/src` | 21 | 3.968 (−8) |
| `apps/web/src` (`.ts`/`.tsx`) | 99 (+40) | 11.188 (+4.017) |
| `apps/web/src/styles` (CSS) | 4 | 1.907 (+464) |

Fichero más largo de `apps/web/src`: `view-models/forms/specs.ts`, **630 líneas**, con su razón escrita (es la tabla de los catorce formularios). Ningún `.ts`/`.tsx` pasa de 250 líneas sin un `LINE BUDGET:` en la cabecera, y un test lo comprueba.

**Componentes nuevos frente a reutilizados**: 4 de infraestructura (`DataTable`, `ErrorView`, `AsOfPicker`, `DuplicateDialog`) más 6 de gráficas (`Chart`, `ChartLegend`, `ChartTable`, `RangeButtons`, `SeriesCard`, `Allocation`) y `FormFields`. Los nueve componentes de la 006 (`Amount`, `Badge`, `Callout`, `Dialog`, `EmptyState`, `Field`, `Figure`, `Section`, `Skeleton`) se han reutilizado tal cual: **ninguno se ha clonado**. `MovementList` dejó de tener su propio par tarjeta/tabla y pasó a usar `DataTable`, que es lo que la generalización buscaba.

### Comprobado en el navegador

- **Cero desplazamiento horizontal en 112 combinaciones** (16 rutas × 7 anchuras: 320, 360, 400 con DPR 3, 768, 1023, 1024 y 1280), con la privacidad **quitada**, que es el caso ancho.
- **Cero errores de consola** en las 112.
- Navegación atrás: la fecha vive en la URL y atrás deshace el último cambio sin salir de la pantalla; `Escape` cierra un diálogo sin mover la ruta; atrás desde el detalle vuelve a Movimientos.
- Las tres gráficas se leen en vertical a 400×890 DPR 3.
