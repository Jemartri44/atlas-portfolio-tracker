...
- **N17 — Pendiente de decisión: la cantidad del aviso de compra previa tras un contrasplit (tanda fiscal, punto 7a).** El aviso cita la cantidad **comprada**, en las unidades de la compra: tras un contrasplit 1:4 dice «2 títulos» cuando quedan 0,5. Hay dos arreglos y **los dos no mueven lo mismo**:
  - **(a) Citar lo que de esa compra sigue en cartera** (la suma de sus lotes abiertos). Arregla el contrasplit y además dice la verdad cuando la venta consumió parte de la compra. **Mueve la instantánea en un aviso**: el de la compra de `ast_world` del 2026-09-04, que pasa de 6,3526 a **0,0551** (en el detalle y en el mensaje). Lo comprobé aplicándolo y regenerando: nada más se mueve.
  - **(b) Citar la compra en las unidades de hoy** (la cantidad comprada por los `scale` posteriores). No mueve la instantánea (ningún aviso del libro sintético tiene un `scale` entre la compra y la venta), pero necesita que la proyección guarde los ratios por activo, con su propio deshacer en las acciones corporativas fallidas, y con una fracción como 1/3 el resultado deja de ser exacto.
  Como la orden era parar si la instantánea se movía, **no he aplicado ninguna**. Mi recomendación es la (a).
...
- **N27 — El paquete de la web** mide ahora **167,7 KB de 168** (el arranque, 77,7 de 80): lo nuevo son las traducciones de `duplicate_isin` y `tax_duplicate_isin`. Al rebasar sobre la rama de defectos tomaré el valor que mida el *build*, como pidió la dirección.
