...
- **N40 — `24:etc_gain` lleva certeza baja, no media** (decisión de la dirección). Leer un ETC como ganancia patrimonial va contra una consulta vinculante que dice «en todo caso»: esa lectura es débil. `24:etp_gain` se queda en media, porque ahí la estructura jurídica de cada producto deja margen real. La baja no está en la fila del documento, así que `24:etc_gain` se exime también en la certeza, con su motivo; la más dudosa de la fila (media) la siguen llevando las dos variantes del ETP. Predicción en `etc-gain-certainty-expectation.md`: se movió una línea de `synthetic-v1.tax.json`, el campo `certainty` de ese dudoso, y nada más.
