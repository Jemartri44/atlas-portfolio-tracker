...
## 12. Tercera pasada de la revisión de la PR #75 (2026-09-24)

### 12.1 El bloqueante: dos confirmaciones a la vez resucitaban el borrador — decisión de la dirección

- **Qué pasaba** (reproducido por el revisor): el cerrojo cubre cada escritura, no la secuencia sellar-registrar-quitar, y `update` escribía sin condición. La consola 1 sellaba A, registraba A y quitaba el borrador; la consola 2, que lo había leído antes, sellaba B **recreando el fichero** y su registro se rechazaba por huella: quedaba un borrador fantasma con B (y, con el sí al duplicado, dos compras). En la web, un borrador desaparecido hacía que `confirmDraft` devolviera `undefined` y `writeStep` registrara la operación **como nueva**.
- **Decisión de la dirección**: sellar es una operación condicional, y confirmar un borrador que ya no existe no registra nada. `update(draft, readStamp)` solo escribe si el borrador **sigue existiendo** y no tiene sello o tiene **exactamente el leído**; se comprueba y se escribe **bajo el cerrojo** (consola) y **en una sola transacción** (IndexedDB). Si no, `DraftChangedError` (`draft_changed`, «ya no está» o «lo está confirmando otro»), traducido en las dos interfaces, y la confirmación para sin escribir nada. Un sello que ya trae el borrador se **reutiliza**, siempre sellado de nuevo con la misma condición. En la web, un borrador que ya no está se dice y se para: `confirmDraft` ya no devuelve `undefined` y `writeStep` no tiene camino a «registrar como nueva».
