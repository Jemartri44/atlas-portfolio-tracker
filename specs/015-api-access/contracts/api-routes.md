...
|---|---|---|
| `GET /api/auth/login[?device_id=<22>]` | ninguna. Una cookie de sesión o un token presentes se **ignoran** (un inicio de sesión nuevo sustituye al anterior) | `302` a Google con `response_type=code`, `client_id`, `redirect_uri=<ATLAS_ORIGIN>/api/auth/callback`, `scope=openid email`, `state`, `nonce`, `code_challenge` (S256) y `code_challenge_method=S256`. Pone `Set-Cookie: __Host-atlas_login=…` (data-model §1.3). Un `device_id` con otro formato se descarta en silencio, y la API asigna uno nuevo: no es credencial (§7 P1) |
